...
const SERVICES = [
  ["Document Preparation & Management", "We organize your existing business documents, maintain a clean digital filing system, and prepare routine administrative documents — trackers, checklists, internal forms, and reports — from information you provide."],
  ["Invoice Administration", "We prepare and send invoices when authorized, record payment status, track issue and due dates, and keep your invoice records organized and current. We do not perform debt collection."],
  ["License & Renewal Tracking", "We record your licenses, permits, registrations, certifications, and insurance documentation, then track expiration and renewal dates so nothing quietly lapses. We track this information administratively — we do not determine what your business legally requires."],
  ["Vendor Administration", "We maintain vendor contact records, organize W-9s and Certificates of Insurance, track insurance expiration dates, and prepare routine vendor paperwork using information you approve."],
  ["CRM & Data Management", "We enter and update customer and vendor records, clean up duplicates, maintain spreadsheets, and handle routine data entry so your systems stay accurate."],
  ["Project Administration", "We create and maintain project folders and trackers, organize project documents, update project status, and prepare routine administrative reports for work in progress."],
  ["Forms & Paperwork", "We prepare routine business forms, applications, checklists, and internal paperwork using information you provide. Anything requiring licensed professional judgment is handled by the appropriate qualified professional — not by us."],
  ["Data Entry & Reporting", "Spreadsheets, data cleanup, status reports, and monthly operational summaries — the recurring reporting that keeps you informed without consuming your time."],
  ["General Administrative Support", "Routine administrative work within your approved Scope of Services. Every plan has a defined scope and reserved capacity, which protects both sides of the relationship."],
];
      ...
      <div className="nav-inner">
        ...
        <nav className="nav-links">
          {items.map((it) => (
            <button
              key={it}
              className={"nav-link" + (page === it ? " active" : "")}
              onClick={() => setPage(it)}
            >
              {it}
            </button>
      ...
      {open && (
        <div className="nav-mobile">
          {items.map((it) => (
            <button
              key={it}
              className={"nav-mobile-link" + (page === it ? " active" : "")}
      ...
      <section className="section">
        ...
        <div className="plain-grid">
          {SERVICES.map(([title], i) => (
            <div className="plain-grid-item" key={title}>
      ...
      <section className="section">
        {SERVICES.map(([title, text], i) => (
          <div className="service-row" key={title}>
            <h3>{String(i + 1).padStart(2, "0")} &middot; {title}</h3>
            <p>{text}</p>
          </div>
        ))}
      </section>
    ...
    <div className="app">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .plain-grid-item { display: flex; gap: 0.7rem; align-items: baseline; padding: 0.6rem 0; border-bottom: 1px solid #E4E9EF; font-size: 0.92rem; color: ${COLORS.navy}; }
